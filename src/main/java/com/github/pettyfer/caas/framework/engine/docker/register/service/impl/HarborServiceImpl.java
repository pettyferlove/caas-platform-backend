...
public class HarborServiceImpl implements IHarborService {
    ...
    public List<RepositoryView> queryRepository(String projectId, String q) {
        ...
        ResponseEntity<List<Repository>> responseEntity = restTemplate.exchange(globalConfiguration.getDockerRegistryPath() + Repository.URL_GET + "?project_id={project_id}&q={q}&page_size={page_size}", HttpMethod.GET, requestEntity, parameterizedTypeReference, params);
    ...
    public List<RepositoryTagView> queryRepositoryTag(String repoName) {
        ...
        URI uri = new URI(globalConfiguration.getDockerRegistryPath() + MessageFormat.format(DetailedTag.URL, cn.hutool.core.codec.Base64.decodeStr(repoName)));
    ...
    public Boolean checkProject(String projectName) {
        try {
            ...
            HttpEntity<MultiValueMap<String, String>> requestEntity = new HttpEntity<>(headers);
            ResponseEntity<String> responseEntity = restTemplate.exchange(globalConfiguration.getDockerRegistryPath() + Project.URL + "?project_name={project_name}", HttpMethod.HEAD, requestEntity, String.class, params);
    ...
    public String createProject(ProjectCreate project) {
        if (!checkProject(project.getProjectName())) {
            ...
            ResponseEntity<String> responseEntity = restTemplate.postForEntity(globalConfiguration.getDockerRegistryPath() + Project.URL, requestEntity, String.class);
    ...
    private String findProject(String projectName){
        ...
        ResponseEntity<List<Project>> listResponseEntity = restTemplate.exchange(globalConfiguration.getDockerRegistryPath() + Project.URL + "?name={project_name}", HttpMethod.GET, queryEntity, parameterizedTypeReference, params);
